...
from .serializers import  SalesOrderSerializer,DealerSerializer
...
class DealerList(APIView):
    ...
    template_name = 'accounts/api_dealer_list.html'
...
class DealerCreate(APIView):
    renderer_classes = [TemplateHTMLRenderer]
    template_name = 'accounts/api_dealer.html'
...
class DealerUpdate(APIView):
    renderer_classes = [TemplateHTMLRenderer]
    template_name = 'accounts/api_dealer_update.html'

    def get(self, request, pk, format=None):
        d = get_object_or_404(Dealer, dealer_code=pk)
        ds = DealerSerializer(d)
        return Response({'serializer': ds,"dealer":d})
